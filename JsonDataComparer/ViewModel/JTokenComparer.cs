...
namespace JsonDataComparer.ViewModel
{
    public class JTokenComparer : IEqualityComparer<JToken>
    {
        ...
        private bool Equals(JContainer container1, JContainer container2)
        {
            ...
            bool equals = true;
            if (t1.Count == t2.Count && t1.Count == 1)
            {
                var token1 = t1[0];
                var token2 = t2[0];
                ...
                for (int i = 0; i < t1.Count(); i++)
                {
                    ...
                    switch (token1)
                    {
                        case JObject object1:
                            ...
                            var keys = new HashSet<string>(comparisonRules.Where(c => !c.Ignore).Select(c => c.Name)
                                .Distinct());
                            var properties1 = object1.Properties().Where(p => keys.Contains(p.Name)).ToList();
                            if (!properties1.Any())
                            {
                                Log("Keys not found for Path {0}", token1.Path);
                                return false;
                            }
        ...
        private bool IsIgnored(JToken token)
        {
            ...
            {
                xpath = path.Substring(0, indexDot);
                key = path.Substring(indexDot + 1);
            }
